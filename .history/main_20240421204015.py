...
def compress_prompt_4(original_prompt, tokenizer=None):
    ...
    *demonstrations, question = original_prompt.split("\n\n")
    demonstrations = [sentence for sentence in demonstrations if sentence not in sentences]
    words_pattern = r'\b(?:' + '|'.join(words) + r')\b'
    demonstrations = [re.sub(words_pattern, '', demo, flags=re.IGNORECASE) for demo in demonstrations]
    compressed_prompt = "\n\n".join(demonstrations) + "\n\n" + question
...
def compress_prompt_all(original_prompt, tokenizer=None):
    *demonstrations, question = original_prompt.split("\n\n")
    demonstrations = [compress_prompt_4(i) for i in demonstrations]
    demonstrations = [compress_prompt_3(i) for i in demonstrations]
    demonstrations = [compress_prompt_1(i) for i in demonstrations]
    compressed_prompt = "\n\n".join(demonstrations) + "\n\n" + question
