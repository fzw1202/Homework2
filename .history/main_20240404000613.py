...
def load_dataset(demo_type):
    ...
    assert demo_type == 'all' or demo_type == 'complex' or demo_type == 'easy' or demo_type == 'mid'
    if demo_type != 'all':
...
def compress_prompt(original_prompt, tokenizer=None):
    """
    Compress the given prompt to 
    """

    ######################################
    # Implement prompt compression here! #
    ######################################

    # Example: trivial compression keeping only the first 2 demonstrations
...
def generate_answer(prompts, model):
    ...
    for input_text in tqdm(prompts):
        output = model.generate(input_text, use_tqdm=False, sampling_params=sampling_params)
        answer = output[0].outputs[0].text.split('\n\n')[0] # only keep the first paragraph
...
if __name__ == '__main__':
    ...
    # """Compress the prompt"""
    prompts = [compress_prompt(prompt) for prompt in prompts]
    print(prompts[0], end='')

    """Conduct Inference"""
    answers = generate_answer(prompts, model)

    """Evaluate the answers"""
    scores = evaluate_answers(answers, labels)

    """Save the results"""
    output_path = f"results/{args.model_name}_{args.demo_type}.json"
