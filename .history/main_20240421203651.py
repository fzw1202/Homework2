...
def compress_prompt_4(original_prompt, tokenizer=None):
    ...
    *demonstrations, question = original_prompt.split("\n\n")
    ...
    demonstrations = [sentence for sentence in demonstrations if sentence not in sentences_to_delete]

    demonstrations = r'\b(?:' + '|'.join(words) + r')\b'

    filtered_demonstrations = [re.sub(words_pattern, '', demo, flags=re.IGNORECASE) for demo in filtered_sentences]
